...
public class ListaEnlazada {
	...
	public double eliminarElementoEspecifico(double dato) {
		...
		if(verificarListaVacia()) {
			System.out.println("No hay elementos para eliminar");
			return -1;
		}else if(Double.parseDouble(df.format(nodoInicio.getDato().getTemperatura())) == dato) {
			...
			while(Double.parseDouble(df.format(nodoSigiente.getDato().getTemperatura()))!=dato && nodoSigiente.getEnlace() != null) {
				
				nodoAnterior = nodoAnterior.getEnlace();
				nodoSigiente = nodoSigiente.getEnlace();
			}
			if(Double.parseDouble(df.format(nodoSigiente.getDato().getTemperatura())) == dato) {
